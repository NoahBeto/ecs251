...
class ComprehensiveVisualizer:
    def __init__(self, results_dir='benchmark_results_iouring_1MB'):
    ...
    def load_all_data(self):
        ...
        # Throughput vs concurrency (1MB file)
        for c in [10, 50, 100, 500, 1000, 5000, 10000]:
            data = self.parse_ab(f"{self.results_dir}/iouring_c{c}_1mb.txt")
            if 'rps' in data:
    ...
    def plot_latency_extended(self):
        ...
        ax.set_title('Latency vs File Size (1MB)\n100 Concurrent Connections',
                    fontsize=15, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3, linestyle='--')
        ax.set_xscale('log')
        ax.legend(fontsize=12)
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            # 1. Throughput
            f.write("1. THROUGHPUT vs CONCURRENCY (1MB file)\n")
            f.write("-" * 80 + "\n")
            if self.throughput:
