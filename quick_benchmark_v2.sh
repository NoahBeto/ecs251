...
# Comprehensive benchmark for io_uring file server
...
for c in 10 50 100 500 1000 5000 10000; do
    ...
    # Run benchmark
    ab -n $n_requests -c $c -s $timeout -q http://localhost:8000/test_data/1kb.txt 2>&1 > "${OUTPUT_DIR}/iouring_c${c}_1kb.txt"
    AB_EXIT=$?
    ...
    if [ $AB_EXIT -eq 0 ]; then
        cat "${OUTPUT_DIR}/iouring_c${c}_1kb.txt" | \
            grep -E "Requests per second|Time per request|Failed requests"
        echo "Peak CPU: ${peak_cpu}%"
        echo "Peak CPU: ${peak_cpu}%" >> "${OUTPUT_DIR}/iouring_c${c}_1kb.txt"
    else
