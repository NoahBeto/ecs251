...
/* ===== syscall counters for analysis ===== */
static unsigned long long io_uring_submit_count = 0;
static unsigned long long io_uring_enter_count  = 0;
static unsigned long long http_request_count    = 0;

/* Track whether we need to submit at end of current event loop iteration */
static int pending_submissions = 0;

static int submit_and_count(void);
...
/* ===== Wrapped submit that records syscall usage ===== */
static int submit_and_count() {
    int ret = io_uring_submit(&ring);
    if (ret >= 0) {
        io_uring_submit_count++;
        io_uring_enter_count++;  /* submit triggers io_uring_enter */
    }
    return ret;
}
...
void sigint_handler(int signo) {
    ...
    FILE *f = fopen("iouring_syscalls.txt", "w");
    if (f) {
        fprintf(f, "http_requests %llu\n", http_request_count);
        fprintf(f, "io_uring_submit %llu\n", io_uring_submit_count);
        fprintf(f, "io_uring_enter %llu\n", io_uring_enter_count);

        if (http_request_count > 0) {
            fprintf(f, "syscalls_per_request %.4f\n",
                (double)io_uring_enter_count / http_request_count);
        }

        fclose(f);
    }

    printf("HTTP requests:        %llu\n", http_request_count);
    printf("io_uring_submit:      %llu\n", io_uring_submit_count);
    printf("io_uring_enter:       %llu\n", io_uring_enter_count);

    if (http_request_count > 0) {
        printf("syscalls/request:    %.4f\n",
            (double)io_uring_enter_count / http_request_count);
    }
