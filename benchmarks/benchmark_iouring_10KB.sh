...
# Benchmark for io_uring file server - 10KB FILE ONLY
# Based on ECS 251 evaluation plan

OUTPUT_DIR="benchmark_results_iouring_10KB"
mkdir -p $OUTPUT_DIR

echo "========================================"
echo "10KB File Performance Benchmark"
echo "========================================"
echo ""
...
# ---------------------------------------------------------------
# Test 1: Throughput vs Concurrency (10KB file)
...
# ---------------------------------------------------------------
echo "=== Test 1: Throughput vs Concurrency (10KB file) ==="
for c in 10 50 100 500 1000 5000 10000; do
    echo "Concurrency: $c"

    SERVER_PID=$(pgrep -f fileserver_iouring)
    ...
    ab -n $n_requests -c $c -s $timeout -k \
        -q http://localhost:8000/test_data/10kb.txt \
        2>&1 > "${OUTPUT_DIR}/iouring_c${c}_10kb.txt"
    AB_EXIT=$?

    if [ $AB_EXIT -eq 0 ]; then
        grep -E "Requests per second|Time per request|Failed requests" \
            "${OUTPUT_DIR}/iouring_c${c}_10kb.txt"
    else
...
for file in 1kb.txt 10kb.txt 100kb.txt 1mb.txt 10mb.txt; do
    ...
    ab -n 2000 -c 500 -q \
        ...
        2>&1 | tee "${OUTPUT_DIR}/iouring_c500_${file}" \
...
if command -v perf &>/dev/null; then
    ...
    sleep 1
    ab -n 10000 -c 500 -q \
        http://localhost:8000/test_data/10kb.txt >/dev/null 2>&1
    wait $PERF_PID
...
# ---------------------------------------------------------------
echo "=== Test 5: System Call Analysis (10KB file) ==="
...
# ---------------------------------------------------------------
echo "=== Test 7: CPU Utilization vs Concurrency (10KB file) ==="
...
# # Benchmark for io_uring file server
# # Fixes:
# #   1. n_requests scales with concurrency (not fixed 20000)
# #   2. CPU measured with pidstat (instantaneous, not ps cumulative average)
# #   3. Syscall test re-runs with new binary
# #   4. Test 7 CPU uses pidstat too
...
# # Test 7: CPU Utilization vs Concurrency (10KB file)
# echo "=== Test 7: CPU Utilization vs Concurrency (10KB file) ==="
...
#     ab -n 10000 -c $c -q http://localhost:8000/test_data/10kb.txt >/dev/null 2>&1 &
#     AB_PID=$!
...
# # Test 1: Throughput vs Concurrency (10KB file ONLY)
# echo "=== Test 1: Throughput vs Concurrency (10KB file) ==="
# for c in 10 50 100 500 1000 5000 10000; do
#     echo "Concurrency: $c"
...
#     # Run benchmark on 10KB file
#     ab -n $n_requests -c $c -s $timeout -q http://localhost:8000/test_data/10kb.txt 2>&1 > "${OUTPUT_DIR}/iouring_c${c}_10kb.txt"
#     AB_EXIT=$?
...
#     if [ $AB_EXIT -eq 0 ]; then
#         cat "${OUTPUT_DIR}/iouring_c${c}_10kb.txt" | \
#             grep -E "Requests per second|Time per request|Failed requests"
#         echo "Peak CPU: ${peak_cpu}%"
#         echo "Peak CPU: ${peak_cpu}%" >> "${OUTPUT_DIR}/iouring_c${c}_10kb.txt"
#     else
...
# for file in 1kb.txt 10kb.txt 100kb.txt 1mb.txt 10mb.txt; do
#     echo "File: $file"
#     ab -n 2000 -c 500 -q http://localhost:8000/test_data/${file} 2>&1 | \
#         tee "${OUTPUT_DIR}/iouring_c100_${file}" | \
#         grep -E "Requests per second|Time per request|Transfer rate"
#     echo ""
# done
...
#     else
#         ab -n 1000 -c $c -H "Range: bytes=0-1023" \
#             http://localhost:8000/test_data/1mb.txt 2>&1 | \
#             tee "${OUTPUT_DIR}/range_c${c}.txt" | \
#             grep -E "Requests per second|Time per request"
#     fi
#     echo ""
...
# echo "=== Test 4: CPU Profiling (perf) ==="
# if command -v perf &> /dev/null; then
#     echo "Recording CPU profile during 10KB benchmark..."
...
#     sleep 1
#     ab -n 10000 -c 500 -q http://localhost:8000/test_data/10kb.txt >/dev/null 2>&1
...
# # Test 5: System Calls Analysis (10KB file)
# echo "=== Test 5: System Call Analysis (10KB file) ==="
...
# strace -c -f -o ${OUTPUT_DIR}/syscalls_detailed.txt ./build/fileserver_iouring 8000 &
# SERVER_PID=$!

# sleep 2

# echo "Running benchmark on 10KB file (1000 requests)..."
# ab -n 1000 -c 100 -q http://localhost:8000/test_data/10kb.txt >/dev/null 2>&1
...
# echo ""
# echo "Primary test: 10KB file performance"