...
# Benchmark for io_uring file server - 100KB FILE ONLY
# Based on ECS 251 evaluation plan

OUTPUT_DIR="benchmark_results_iouring_100KB"
mkdir -p $OUTPUT_DIR

echo "========================================"
echo "100KB File Performance Benchmark"
echo "========================================"
echo ""
...
# ---------------------------------------------------------------
# Test 1: Throughput vs Concurrency (100KB file)
...
# ---------------------------------------------------------------
echo "=== Test 1: Throughput vs Concurrency (100KB file) ==="
for c in 10 50 100 500 1000 5000 10000; do
    ...
    ab -n $n_requests -c $c -s $timeout -k \
        -q http://localhost:8000/test_data/100kb.txt \
        2>&1 > "${OUTPUT_DIR}/iouring_c${c}_100kb.txt"
    AB_EXIT=$?

    if [ $AB_EXIT -eq 0 ]; then
        grep -E "Requests per second|Time per request|Failed requests" \
            "${OUTPUT_DIR}/iouring_c${c}_100kb.txt"
    else
...
#     sleep 1
#     ab -n 10000 -c 500 -q \
#         http://localhost:8000/test_data/100kb.txt >/dev/null 2>&1
#     wait $PERF_PID
...
# ---------------------------------------------------------------
echo "=== Test 5: System Call Analysis (100KB file) ==="
...
# ---------------------------------------------------------------
echo "=== Test 7: CPU Utilization vs Concurrency (100KB file) ==="
...
echo "Run: python3 visualizations/visualize_results_iouring_100KB.py to generate graphs"
echo "========================================"