...
# Benchmark for io_uring file server - 1MB FILE ONLY
# Based on ECS 251 evaluation plan

OUTPUT_DIR="benchmark_results_iouring_1MB"
mkdir -p $OUTPUT_DIR
sudo -v
...
for c in 10 50 100 500 1000 5000 10000; do
    ...
    ab -n $n_requests -c $c -s $timeout -k \
        -q http://localhost:8000/test_data/1mb.txt \
        2>&1 > "${OUTPUT_DIR}/iouring_c${c}_1mb.txt"
    AB_EXIT=$?

    if [ $AB_EXIT -eq 0 ]; then
        grep -E "Requests per second|Time per request|Failed requests" \
            "${OUTPUT_DIR}/iouring_c${c}_1mb.txt"
    else
...
# ---------------------------------------------------------------
echo "=== Test 7: CPU Utilization vs Concurrency (1MB file) ==="
...
echo "Run: python3 visualizations/visualize_results_iouring_1MB.py to generate graphs"
echo "========================================"